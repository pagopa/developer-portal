...
import { getProductGuidePageBy, getProductGuidePages } from "@/adapters/static/staticProductGuidePage";
...
export const getStaticPaths: GetStaticPaths<Params> = () => ({
  paths: pipe(
    ...
    RA.map(({ product, guideSlug, versionSlug, slug }) => ({
      params: { productSlug: product.slug, guideSlug, guideVersionSlug: versionSlug, guidePageSlug: slug },
    })),
    (array) => [...array]
  ),
  fallback: false,
...
export const getStaticProps: GetStaticProps<ProductGuidePageProps, Params> = (
  ...
  pipe(
    ...
    O.chain(({ productSlug, guideSlug, guideVersionSlug, guidePageSlug }) =>
      getProductGuidePageBy(productSlug, guideSlug, guideVersionSlug, guidePageSlug)
    ),
    O.foldW(
      () => ({ notFound: true }),
      (page) => ({
