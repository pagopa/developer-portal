...
async def query_creation(
    ...
    LOGGER.info(f"trace_id {trace_id} starting processing for user {userId}")
    ...
    LOGGER.info(f"trace_id {trace_id} chat_generate done.")
    answer = chatbot.get_final_response(answer_json)
    LOGGER.info(f"trace_id {trace_id} get_final_response done.")
    ...
    if can_evaluate():
        ...
        LOGGER.info(f"trace_id {trace_id} background_task for evaluate launched.")
    ...
    try:
        ...
        LOGGER.info(f"trace_id {trace_id} write to dynamodb.")
