...
def mock_signup_user(username: str, password: str) -> dict:
    user_pool_id = SETTINGS.auth_cognito_userpool_id

    client_response = client_cognito.create_user_pool_client(
        UserPoolId=user_pool_id, ClientName=f"test_client_{username}"
    )
    client_id = client_response["UserPoolClient"]["ClientId"]
    ...
    if users["Users"] == []:
        client_cognito.sign_up(ClientId=client_id, Username=username, Password=password)
