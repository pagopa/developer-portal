...
def add_langfuse_score_query(
    ...
    if query_feedback.badAnswer is not None:
        bad_answer = -1 if query_feedback.badAnswer else 0
        comment = query_feedback.feedback.user_comment if query_feedback.feedback else None
        payload = {
            "operation": "add_score",
            "data": {
                "trace_id": query_id,
        ...
        LOGGER.info(f"sqs response query_feedback.feedback.user_comment: {sqs_response}")

    if (
        query_feedback.feedback and
        query_feedback.feedback.user_response_relevancy is not None
        ):
        ...
        LOGGER.info(f"sqs response query_feedback.feedback.user_response_relevancy: {sqs_response}")

    if (
        query_feedback.feedback and
        query_feedback.feedback.user_faithfullness is not None
        ):
        ...
        LOGGER.info(f"sqs response query_feedback.feedback.user_faithfullness: {sqs_response}")
