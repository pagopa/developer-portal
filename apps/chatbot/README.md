...
This folder contains all the details to build a multi-agentic system, named **Discovery**, (a system composed of multiple collaborating AI agents) with RAG (Retrieval-Augmented Generation, which enhances responses by retrieving relevant information from external sources) using the documentation provided in [`PagoPA Developer Portal`](https://developer.pagopa.it/).
