...
describe('sanitizeFilename', () => {
  it('replaces illegal characters', () => {
    expect(sanitizeUrlAsFilename('file/name?with*illegal|chars.txt')).toBe('file-name-with-illegal-chars.txt');
  });

  it('replaces control characters', () => {
    expect(sanitizeUrlAsFilename('file\u0000name\u0001.txt')).toBe('file-name-.txt');
  });
  ...
  it('uses the replacement option', () => {
    expect(sanitizeUrlAsFilename('file/name', { replacement: '-' })).toBe('file-name');
  });
  ...
  it('returns unchanged valid filename', () => {
    expect(sanitizeUrlAsFilename('valid-filename.txt')).toBe('valid-filename.txt');
  });
