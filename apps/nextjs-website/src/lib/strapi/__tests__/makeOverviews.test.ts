...
describe('makeOverviews', () => {
  ...
  it('should log an error and skip overview with empty product slug', () => {
    const result = makeOverviews(overviewWithEmptyProductSlug());
    expect(result).toHaveLength(0);
  ...
  it('should log an error and skip tutorials with missing product slug', () => {
    const result = makeOverviews(overviewWithMissingTutorialProductSlug());
    expect(result).toHaveLength(1);
  ...
  it('should log an error and skip tutorials with missing product slug', () => {
    const result = makeOverviews(overviewWithMissingTutorialSlug());
    expect(result).toHaveLength(1);
    expect(result[0].tutorials?.list).toHaveLength(0);
    expect(consoleSpy).toHaveBeenCalledWith(
  ...
  it('should log an error and skip guides with empty product slug', () => {
    const result = makeOverviews(overviewWithEmptyGuideProductSlug());
    expect(result).toHaveLength(1);
