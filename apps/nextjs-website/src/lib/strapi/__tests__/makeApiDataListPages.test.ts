...
describe('makeApiDataListPages', () => {
  it('should transform strapi api data list pages to api data list page template props', () => {
    const result = makeApiDataListPages(_.cloneDeep(strapiApiDataListPages));
  ...
  it('should handle minimal data with missing optional fields', () => {
    const result = makeApiDataListPages(_.cloneDeep(minimalApiDataListPages()));
    expect(result).toHaveLength(1);
  ...
  it('should handle empty data array', () => {
    const result = makeApiDataListPages(emptyApiDataListPages());
    expect(result).toHaveLength(0);
  });

  it('should handle page with empty api data', () => {
    const result = makeApiDataListPages(apiDataListPageWithEmptyApiData());
    expect(result).toHaveLength(1);
  ...
  it('should handle mixed API types and filter invalid ones', () => {
    const result = makeApiDataListPages(apiDataListPageWithMixedApiTypes());
    expect(result).toHaveLength(1);
    const firstElement = result[0];
  ...
  it('should handle page without description', () => {
    const result = makeApiDataListPages(apiDataListPageWithoutDescription());
    expect(result).toHaveLength(1);
    expect(result[0].hero.subtitle).toBe('');
  });

  it('should filter out invalid API data', () => {
    const result = makeApiDataListPages(apiDataListPageWithInvalidApiData());
    expect(result).toHaveLength(1);
    const firstElement = result[0];
  ...
  it('should handle multiple pages', () => {
    const result = makeApiDataListPages(multipleApiDataListPages());
  ...
  it('should correctly set hero properties', () => {
    const result = makeApiDataListPages(strapiApiDataListPages);
  ...
  it('should handle API data with missing icon', () => {
    const result = makeApiDataListPages(minimalApiDataListPages());
  ...
  it('should prioritize REST slug over SOAP slug in apiDetailSlugs', () => {
    const result = makeApiDataListPages(apiDataListPageWithBothRestAndSoap());
