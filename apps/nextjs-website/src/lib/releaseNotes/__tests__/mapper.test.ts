...
import { mapReleaseNotesProps } from '@/lib/releaseNotes/mapper';
...
describe('mapReleaseNotesProps', () => {
  ...
  it('should transform strapi release notes to release note page props', () => {
    const result = mapReleaseNotesProps('it', _.cloneDeep(strapiReleaseNotes));
  ...
  it('should handle minimal data with missing optional fields', () => {
    const result = mapReleaseNotesProps(
      'it',
  ...
  it('should handle empty data array', () => {
    ...
    const result = mapReleaseNotesProps('it', emptyData);
    expect(result).toHaveLength(0);
  });

  it('should use product banner links when release note banner links are empty', () => {
    const result = mapReleaseNotesProps('it', releaseNotesWithoutBannerLinks());
  ...
  it('should handle release notes with product that has undefined banner links', () => {
    const result = mapReleaseNotesProps('it', minimalDataReleaseNotes());
    expect(result[0].bannerLinks).toBeUndefined();
  });
