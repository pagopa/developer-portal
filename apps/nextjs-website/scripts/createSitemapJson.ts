...
async function fetchStrapiGuides(): Promise<StrapiGuide[]> {
    try {
        ...
        console.log(`Successfully fetched ${data.data?.length || 0} guides from Strapi`);
        return data.data || [];
    } catch (error) {
...
function generateUrlPathWithStrapiData(
    ...
    const productSlug = guide.attributes.product?.data?.attributes?.slug || 'default-product';
...
async function main() {
    try {
        ...
        console.log(`Found ${Object.keys(dirStructure).length} directories with Markdown files`);
        console.log(`Mapped ${Object.keys(dirToGuideMap).length} directories to Strapi guides`);
        ...
        for (const dirName of Object.keys(dirStructure)) {
            ...
            if (!summaryFile) {
                console.log(`No SUMMARY.md found for directory: ${dirName}, skipping...`);
                continue;
            }
            ...
            for (const contentFile of dirStructure[dirName].contentFiles) {
                try {
                    ...
                    const urlPath = generateUrlPathWithStrapiData(dirName, contentFile, strapiGuides, dirToGuideMap);

                    if (!urlPath) {
                        console.log(`Skipping file ${contentFile} - no valid URL path could be generated`);
                        continue;
                    }
                    ...
                    console.log(`Added sitemap item: ${title} (${dirName}) with path: ${urlPath}`);
                } catch (error) {
