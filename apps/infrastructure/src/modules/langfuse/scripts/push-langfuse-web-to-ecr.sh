...
DOCKER_HUB_IMAGE="langfuse/langfuse:${LANGFUSE_VERSION}"
...
echo -e "${BLUE}Langfuse Web ECR Migration Script${NC}"
echo "=================================="
echo ""
...
ECR_REPO_NAME="langfuse-web"
ECR_IMAGE="${ECR_REGISTRY}/${ECR_REPO_NAME}:${LANGFUSE_VERSION}"

echo -e "${GREEN}Step 1:${NC} Logging in to ECR..."
aws ecr get-login-password --region ${AWS_REGION} | docker login --username AWS --password-stdin ${ECR_REGISTRY}

echo -e "\n${GREEN}Step 2:${NC} Pulling Langfuse Web image from Docker Hub..."
docker pull --platform linux/amd64 ${DOCKER_HUB_IMAGE}
...
echo -e "\n${GREEN}✓ Success!${NC} Langfuse Web image pushed to ECR:"
echo "  ${ECR_IMAGE}"
echo ""
echo "Next steps:"
